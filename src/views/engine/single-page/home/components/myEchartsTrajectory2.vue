...
export default {
  ...
  methods: {
      ...
      let option = {
        ...
        tooltip: {
          ...
          formatter: function(params) {
            ...
            let str = Params.name + '<br>' + `实际妥投率：${data1.prescRate}%` + '<br>' + `装车时间：${data1.loadTime}` + '<br>' + temp1
            return str
          }
        },
        legend: {
            ...
            itemStyle: {
              normal: {
                ...
                color: function(params) {
                  ...
                  let newArr = []// 重组数据
                  arrCountry.forEach((item2, idx) => { // 把相同国家的数据放一起了，方便后面再次循环
                    let obj = {
                      country: item2,
                      list: []
                    }
    ...
    changeCountry8(val) { // 改变国家
      ...
      _this.tableData8 = arrSet.slice(0, 10)
      _this.data.data8 = arrSet
    ...
    getYesterTrajectory() { // 首页昨日标准追踪数据查询
      ...
      home.searchYesterTrajectory().then(res => {
        if (res.data.success) {
          ...
          _this.initBar8dataGroup = arr.map(item => item.twowordname).slice(0, 2)
